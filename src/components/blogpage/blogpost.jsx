  ...
  return (
    <div className="grid items-center bg-white w-full rounded-lg shadow-lg h-64 grid-cols-12 grid-rows-12">
      <span className="grid h-12 grid-cols-12 justify-items-center align-middle w-full row-start-1 row-span-2 col-start-1 col-span-12 overflow-hidden">
        ...
        <code className="col-span-3 col-start-2 h-full w-full ">
          By {blog.author}
        </code>
        <code className="col-span-8 col-start-5 h-full w-full">
          On {blog.date}
        </code>
      </span>
      <h2 className="text-3xl text-left mt-2 ml-5 font-bold row-start-3 row-span-3 col-span-12 align-left whitespace-pre-line">
        {blog.topic}
      </h2>
      <p className="text-sm break-words text-wrap md:text-balance mt-2 text-clip overflow-hidden row-start-6 row-span-5 col-span-12 whitespace-pre-line line-clamp-3">
        {blog.body}
      </p>
    </div>
  );
